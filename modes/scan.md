...
5. **Level 2 — ATS APIs / feeds** (parallel):
   ...
   d. For **BambooHR**, the list only has basic metadata. For each relevant item, read `id`, GET `https://{company}.bamboohr.com/careers/{id}/detail`, and extract the full JD from `result.jobOpening`. Use `jobOpeningShareUrl` as public URL if available; otherwise use the detail URL.
...
7.5. **Verify liveness of WebSearch results (Level 3)** — BEFORE adding to pipeline:
   ...
   **Don't interrupt the entire scan if one URL fails.** If `browser_navigate` errors (timeout, 403, etc.), mark as `skipped_expired` and continue with the next.
...
If an inaccessible URL is found:
...
Every company in `tracked_companies` must have `careers_url` — the direct URL to its job listings page. This avoids searching for it every time.
...
Fallback: if you only have the ATS direct URL, navigate to the company website first and find their corporate careers page. Only use the ATS URL directly if the company has no corporate careers page.
