...
async function generatePDF() {
  ...
  // Validate format (CLI uses lowercase; Playwright expects Title Case, e.g. Letter / A4)
  const validFormats = ['a4', 'letter'];
  if (!validFormats.includes(format)) {
    console.error(`Invalid format "${format}". Use: ${validFormats.join(', ')}`);
    process.exit(1);
  }
  const playwrightPaperFormat = format === 'letter' ? 'Letter' : 'A4';

  console.log(`📄 Input:  ${inputPath}`);
  console.log(`📁 Output: ${outputPath}`);
  console.log(`📏 Format: ${playwrightPaperFormat}`);
  ...
  try {
    ...
    // `networkidle` often hangs on real pages (fonts, trackers). Local CV HTML only needs `load`.
    ...
    // Generate PDF (Playwright/Chromium expect "Letter" | "A4", not lowercase)
    const pdfBuffer = await page.pdf({
      format: playwrightPaperFormat,
